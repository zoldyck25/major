...
def send_faculty_reset_email(user):
...
{url_for('faculty.reset_token', token=token, _external=True)}
...
def send_faculty_confirm_email(email, token):
...
{url_for('faculty.confirm_email', token=token, _external=True)}
